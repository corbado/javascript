...
const CompleteEmailOTP = () => {
  ...
  return (
    <div className='h-screen flex flex-col items-center justify-center text-center'>
      ...
      <div>We just sent a one time code to EMAIL. <br/> The code expires shortly, so please enter it soon.</div>
      <div className='w-1/2 grid gap-2 mt-3'>
        <RoundedTextInput placeholder='Code (6 digits)' onChange={setCode}/>
        {error !== undefined ? <p className='accent-red-700'>{error}</p> : <></>}
        <FilledButton content='Continue with email' onClick={submit}/>
        <p className='text-center cursor-pointer' onClick={() => navigate(-1)}>Cancel</p>
      </div>
    </div>
  )
}
