...
export function PasskeyOption() {
  ...
  return (
    <div>
      ...
      <button onClick={() => navigateToNextScreen({ showBenefits: true })}>Passkeys</button>
      <button onClick={() => navigateToNextScreen({ maybeLater: true })}>Maybe Later</button>
      <button onClick={appendPasskey}>Append Passkey</button>
      <button onClick={() => navigateToNextScreen({ failure: true })}>Append Passkey Failure</button>
    </div>
  );
}
