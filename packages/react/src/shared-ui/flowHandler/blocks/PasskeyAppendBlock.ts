...
export class PasskeyAppendBlock extends Block<BlockDataPasskeyAppend> {
  ...
  #enrollmentOp: PasskeyOperationEnrollment | undefined;
  ...
  init() {
    ...
    if (this.observeTracker) {
      this.#enrollmentOp = this.observeTracker.passkeyEnrollmentStartable({});
    }
  }
  ...
  async passkeyAppend() {
    ...
    if (res.err) {
      ...
      if (res.val instanceof ExcludeCredentialsMatchError) {
        this.#enrollmentOp?.clientError({ errorCode: 'exclude_credentials_match', error: res.val });
      ...
      this.#enrollmentOp?.clientError({ error: res.val });
      this.updateScreen(ScreenNames.PasskeyError);

      return;
    }

    this.#enrollmentOp?.finished({});
